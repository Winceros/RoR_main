...
//===========================================================
void Beam::initConnection()
{
	context = zmq_ctx_new();
	socket = zmq_socket(context, ZMQ_REQ);
	zmq_connect(socket,"tcp://127.0.0.1:1111");
}

void Beam::send_msg(string& msg)
{ 
	zmq_msg_t req;
	zmq_msg_init_size(&req, msg.length());
	memcpy(zmq_msg_data(&req), msg.c_str(), msg.length());
    zmq_msg_send(&req, socket, 0);
	zmq_msg_close(&req);
}

string Beam::receive_msg()
{   
	zmq_msg_t reply;
    zmq_msg_init(&reply);
    zmq_msg_recv(&reply, socket, 0);
	string msg = string((char*)zmq_msg_data(&reply));
    zmq_msg_close(&reply);
	return msg;
}

void Beam::closeConnection()
{
	zmq_close(socket);
    zmq_ctx_destroy(context);
}

void Beam::autopilotToggle()
{
	if(ap_mode)
	{
		closeConnection();
		...
		{
			SceneNode *node = landmarks[i].first;
			Entity* ent = landmarks[i].second;

			parentNode->detachObject(ent);
			tsm->destroySceneNode(node);
			tsm->destroyEntity(ent);
		}
		
		for(int i=0 ; i < rays.size() ; i++)
		{
			tsm->getRootSceneNode()->detachObject(rays[i]);
			tsm->destroyManualObject(rays[i]);
		}

	} else
	{
		initConnection();
		//sending command to python. he must plan a path and log it in file
		send_msg(string("plan_path"));
		receive_msg();
		Vector3 pos;
		Quaternion rot;
		calculateDriverPos(pos,rot);
		Ogre::Matrix3 mat;
		rot.ToRotationMatrix(mat);
		Radian yRad,pRad,rRad;
		mat.ToEulerAnglesYXZ(yRad, pRad, rRad);
		float yDeg = yRad.valueDegrees();
		float pDeg = pRad.valueDegrees();
		float rDeg = rRad.valueDegrees();
		
		Vector3 refNodeDiff = wheels[0].refnode0->RelPosition - wheels[0].refnode1->RelPosition;
		float angle = atan2(refNodeDiff.x,refNodeDiff.z) * 180 / 3.14159265359;
		float yWheelDeg = angle - yDeg;

		char buff[50];
		sprintf(buff,"%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f",pos.x,pos.y,position.z,yDeg,pDeg,rDeg,yWheelDeg);
		string pos_rot = string(buff);
		send_msg(pos_rot);
		receive_msg();

		//===placing landmarks on terrain
		ifstream f("C:\\Users\\Ildar_Gilmutdinov\\Documents\\Python projects\\RoR_AI\\planned_path.txt");
		string buf;
		getline(f,buf);
		int step = atoi(buf.c_str());
		int i = 0;
		landmarks = std::vector< pair<Ogre::SceneNode*,Ogre::Entity*> >();
		while(getline(f,buf))
		{
			int ind1 = buf.find(',');
			char x_str[10] = "";
			buf.copy(x_str,ind1,0);
			int x = step*atoi(x_str);

			char z_str[10] = "";
			buf.copy(z_str,buf.length()-ind1,ind1+1);
			int z = step*atoi(z_str);

			int y = (int)ceil(hfinder->getHeightAt(x,z))+1;

			char entName[256]="";
			sprintf(entName, "landmark-%d", i);
		...
		for(int i = 0 ; i < k ; i+=2)
		{
			...
			manual->position(0,0,0); 
			manual->end(); 
			tsm->getRootSceneNode()->attachObject(manual);
			rays.push_back(manual);
		...
		mColTools = new MOC::CollisionTools(tsm);
	}
	ap_mode=!ap_mode;
}
...
void Beam::steer(float arg)
{
	this->hydrodircommand = arg;
}

void Beam::accel(float arg)
{
	float maxvelo = 1;	
	maxvelo = std::max(0.2f, 1-fabs(hydrodircommand)) * 70;

	if(engine)
	{		
		{
			if(WheelSpeed < maxvelo)
				engine->autoSetAcc(arg);
			else
				engine->autoSetAcc(0);
		}
	}
...
{
	//==send-state=======
	Vector3 pos; // not necessary already
	Quaternion rot; 
	calculateDriverPos(pos,rot); 
	Ogre::Matrix3 mat;
	rot.ToRotationMatrix(mat);
	Radian yRad,pRad,rRad;
	Radian yRad1,pRad1,rRad1;
	mat.ToEulerAnglesYXZ(yRad, pRad, rRad);
	float yDeg = yRad.valueDegrees();
	float pDeg = pRad.valueDegrees();
	float rDeg = rRad.valueDegrees();
	...
	float headOrient = getHeadingDirectionAngle() - PI/2;
	int radius = 8;
	...
	char buff[50];
	sprintf(buff,"%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f",position.x,position.y,position.z,yDeg,pDeg,rDeg,getWheelSpeed(),yWheelDeg,threatDeg,threatDist);
	send_msg(string("take_cur_state_and_give_control_vector"));
	receive_msg();
	string pos_rot = string(buff);
	send_msg(pos_rot);
	...
	char acc_str[10] = "";
	int ind1 = msg.find(',');
	msg.copy(acc_str,ind1-1,1);
	float acc = atof(acc_str);
