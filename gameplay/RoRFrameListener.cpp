...
//===========================================
					if (INPUTENGINE.getEventBoolValueBounce(EV_TRUCK_AUTOPILOT))
					{
						curr_truck->autopilotToggle();
					}
//===========================================
				}
...
//==SENDING-MAP==========================
	...
	int step = 5;
	string msg = to_string((long long)(terrainxsize/step+ ((int)terrainxsize % step ? 1:0))) +","+to_string((long long)(terrainzsize/step+ ((int)terrainzsize % step ? 1:0)))
		+ "," + to_string((long long)step) + "\n";
	fprintf(f,msg.c_str());
	for(int x=0 ; x < terrainxsize ; x += step)
	{
		msg = "";
		for(int z=0 ; z < terrainzsize ; z += step)
		{
			msg += ("," + to_string((long double)hfinder->getHeightAt(x,z)/step));
		}
		msg += "\n";
		fprintf(f,msg.c_str());
	}
...
//============MYCODE======================================
	if(curr_truck && curr_truck->state==ACTIVATED 
		&& curr_truck->engine && curr_truck->ap_mode)
	{
		curr_truck->AIstep();
	}
//========================================================
